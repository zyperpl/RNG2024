...
uniform highp vec2 lightPos[16];
uniform highp vec2 resolution;
uniform highp float lightStrength[16];

in highp vec2 fragTexCoord;
layout(location = 0) out highp vec4 fragColor;

highp float Bayer2(inout highp vec2 a)
{
    a = floor(a);
...
highp vec3 GetDitheredPalette(inout highp float x, highp float y, highp vec2 pixel)
{
    ...
    highp vec2 param = ((((pixel * 0.5) * 0.5) * 0.5) * 0.5) * 0.5;
    highp float _102 = Bayer2(param);
    highp vec2 param_1 = (((pixel * 0.5) * 0.5) * 0.5) * 0.5;
    highp float _111 = Bayer2(param_1);
    highp vec2 param_2 = ((pixel * 0.5) * 0.5) * 0.5;
    highp float _119 = Bayer2(param_2);
    highp vec2 param_3 = (pixel * 0.5) * 0.5;
    highp float _126 = Bayer2(param_3);
    highp vec2 param_4 = pixel * 0.5;
    highp float _132 = Bayer2(param_4);
    highp vec2 param_5 = pixel;
    highp float _137 = Bayer2(param_5);
    highp float dith = (((((((((_102 * 0.25) + _111) * 0.25) + _119) * 0.25) + _126) * 0.25) + _132) * 0.25) + _137;
...
void main()
{
    ...
    for (int i = 0; i < 16; i++)
    {
        highp vec2 lp = vec2(lightPos[i].x, resolution.y - lightPos[i].y);
        highp vec2 lightDir = normalize(lp - gl_FragCoord.xy);
        highp float lightDist = length(lp - gl_FragCoord.xy) / 40.0;
        highp float lightIntensity = pow(lightDist, -2.0) * lightStrength[i];
    ...
    highp vec3 _278 = GetDitheredPalette(param, param_1, param_2);
    highp vec3 color = _278;
